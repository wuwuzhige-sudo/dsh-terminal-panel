...
export function apply(ctx, config) {
  ...
  let state = { cwd: null, buffer: '', total: 0, running: null, busy: false, rc: null, signaled: false, user: null, host: null, home: null }
  let bashPath = null
  ...
  const handlers = {
    'term-init': async (args) => {
      ...
      return { cwd: state.cwd, user: state.user, host: state.host, home: state.home, busy: state.busy, rc: state.rc, signaled: state.signaled }
    },
    'term-run': async (args) => {
    ...
    'term-read': async (args) => {
      ...
      return { since: state.total, text, lossy, busy: state.busy, cwd: state.cwd, rc: state.rc, signaled: state.signaled }
    },
  }
