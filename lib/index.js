...
export function apply(ctx, config) {
  ...
  let state = { cwd: null, buffer: '', total: 0, running: null, busy: false, rc: null, signaled: false, user: null, host: null, home: null, sudo: false, runTail: '' }
  ...
  const appendOut = (chunk) => {
    ...
    // SSH mode: detect the completion marker in THIS run's output only (the
    // global buffer may still hold markers of previous runs — matching those
    // would prematurely end the current command and EOF its stdin).
    if (state.running && sshTarget) {
      state.runTail = (state.runTail + text).slice(-4096)
      const m = state.runTail.match(/__SXE_DONE__:(-?\d+)/)
      if (m) {
