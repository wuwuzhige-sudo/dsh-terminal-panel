...
.sxe-sftp-list { flex:1; min-height:0; overflow:auto; user-select:text; -webkit-user-select:text; }
.sxe-sftp-row { display:flex; align-items:center; gap:8px; padding:3px 4px; border-radius:4px; }
      ...
      slots.inject('conversation.view', () => slots.register(
        ...
        (props) => {
          ...
          if (!available) {
            ...
            body = sorted.map((entry) => react.createElement('div', { key: entry.name, className: 'sxe-sftp-row' },
              react.createElement('span', { className: 'sxe-sftp-name' + (entry.type === 'dir' ? ' sxe-sftp-name--click' : ''), onClick: entry.type === 'dir' ? () => load(joinPath(path, entry.name)) : undefined, title: entry.type === 'link' && entry.target ? '→ ' + entry.target : entry.name },
                icon(entry.type) + ' ' + entry.name),
              react.createElement('span', { className: 'sxe-sftp-meta' },
                entry.type === 'dir' ? '目录' : fmtSize(entry.size) + ' · ' + entry.mtime),
              entry.type === 'dir'
          ...
          return react.createElement('div', { className: panelCls('sxe-sftp') },
            ...
            newDir
              ...
              : null,
            react.createElement('div', { className: 'sxe-sftp-list' },
              crumbs.length > 0
                ? react.createElement('div', { className: 'sxe-sftp-row' },
                    react.createElement('span', { className: 'sxe-sftp-name sxe-sftp-name--click', onClick: () => load('/') }, '📁 /'),
                    crumbs.map((seg, i) => react.createElement('span', { key: i, className: 'sxe-sftp-name sxe-sftp-name--click', onClick: () => load('/' + crumbs.slice(0, i + 1).join('/')) }, ' / ' + seg)),
